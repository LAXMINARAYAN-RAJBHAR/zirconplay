...
const CATEGORY_SUFFIXES = {
  music: [
    { suffix: "official music video", tag: "music" }, { suffix: "2025", tag: "new" },
    { suffix: "full album", tag: null }, { suffix: "live performance", tag: "live" },
    { suffix: "lyrics", tag: null }, { suffix: "remix", tag: null },
  ],
  gaming: [
    { suffix: "gameplay 2025", tag: "new" }, { suffix: "speedrun", tag: "trend" },
    { suffix: "tips and tricks", tag: null }, { suffix: "walkthrough", tag: null },
    { suffix: "best moments", tag: null }, { suffix: "live stream", tag: "live" },
  ],
  sports: [
    { suffix: "highlights 2025", tag: "new" }, { suffix: "full match", tag: null },
    { suffix: "best moments", tag: "trend" }, { suffix: "live", tag: "live" },
    { suffix: "analysis", tag: null }, { suffix: "reaction", tag: null },
  ],
  tech: [
    { suffix: "review 2025", tag: "new" }, { suffix: "unboxing", tag: null },
    { suffix: "vs comparison", tag: null }, { suffix: "best settings", tag: null },
    { suffix: "explained", tag: null }, { suffix: "shorts", tag: null },
  ],
  news: [
    { suffix: "today", tag: "live" }, { suffix: "breaking news", tag: "trend" },
    { suffix: "explained", tag: null }, { suffix: "live update", tag: "live" },
    { suffix: "2025", tag: "new" }, { suffix: "documentary", tag: null },
  ],
  comedy: [
    { suffix: "compilation", tag: "trend" }, { suffix: "best moments", tag: null },
    { suffix: "reaction", tag: null }, { suffix: "shorts", tag: null },
    { suffix: "try not to laugh", tag: null }, { suffix: "roast", tag: null },
  ],
  cooking: [
    { suffix: "easy recipe", tag: null }, { suffix: "step by step", tag: null },
    { suffix: "5 minute recipe", tag: "trend" }, { suffix: "for beginners", tag: null },
    { suffix: "restaurant style", tag: null }, { suffix: "shorts", tag: null },
  ],
  travel: [
    { suffix: "travel vlog 2025", tag: "new" }, { suffix: "best places", tag: "trend" },
    { suffix: "travel guide", tag: null }, { suffix: "hidden gems", tag: null },
    { suffix: "budget travel", tag: null }, { suffix: "4K", tag: null },
  ],
  default: [
    { suffix: "2025", tag: "new" }, { suffix: "explained", tag: null },
    { suffix: "tutorial", tag: null }, { suffix: "review", tag: null },
    { suffix: "highlights", tag: "trend" }, { suffix: "shorts", tag: null },
  ],
};
...
const Navbar = ({ currentUser, setCurrentUser, setSideNavbarFunc, sideNavbar }) => {
      ...
      if (currentUser) {
        ...
        if (savedPic) {
          ...
          if (pic) {
            ...
            setUserPic(`https://ui-avatars.com/api/?name=${encodeURIComponent(currentUser)}&background=ff0000&color=fff&size=40`);
          }
        }
      } else {
        setUserPic("https://athenabpo.com/wp-content/uploads/2016/09/Headshot-Blank-Person-Circle-300x300.gif");
      }
    };
    loadPic();
  }, [currentUser]);
  ...
  const handleKeyDown = (e) => {
    ...
    if (e.key === "ArrowDown") { e.preventDefault(); setActiveIndex((p) => Math.min(p + 1, allNavItems.length - 1)); }
      ...
      if (activeIndex >= 0 && allNavItems[activeIndex]) doSearch(allNavItems[activeIndex].text);
  ...
  const startVoiceSearch = () => {
    ...
    speak("Please speak now", () => {
      ...
      recognition.onerror = (e) => {
        if (e.error === "not-allowed") { setIsListening(false); alert("Mic blocked. Allow mic in Chrome settings."); }
      };
    ...
    <div className="navbar">
      ...
      <div className="navbar-left">
        ...
        <Link
          ...
          style={{
            ...
            textDecoration: "none", padding: "5px 5px 5px 5px", borderRadius: "12px",
      ...
      <div className="navbar-middle" ref={dropdownRef} style={{ position: "relative" }}>
        ...
        {showDropdown && (suggestionData.history.length > 0 || suggestionData.items.length > 0 || suggestionData.trending.length > 0) && (
          <div style={{
            position: "absolute", top: "48px", left: 0, width: "calc(100% - 52px)",
            background: "#1e1e1e", borderRadius: "0 0 14px 14px",
            boxShadow: "0 12px 32px rgba(0,0,0,0.7)", zIndex: 9999,
            overflow: "hidden", border: "1px solid #333", borderTop: "none",
          }}>
            ...
            {suggestionData.items.length > 0 && (
              <>
                {!suggestionData.category && (
                  <div style={{ fontSize: "11px", color: "#555", padding: "8px 14px 4px", letterSpacing: "0.06em", textTransform: "uppercase" }}>Suggestions</div>
                )}
      ...
      <div className="navbar-right">
          ...
          {showNotifications && (
            <div style={{
              ...
              <div style={{ padding: "12px", textAlign: "center", borderTop: "1px solid #333" }}>
                <span style={{ color: "#3ea6ff", fontSize: "13px", cursor: "pointer" }}
        ...
        {navbarModal && (
          <div className="navbar-modal">
            ...
            {currentUser ? (
              <div className="navbar-modal-option" onClick={handleLogout} style={{ color: "#ff4444" }}>🚪 Logout</div>
            ) : (
              <div className="navbar-modal-option" onClick={() => { setLogin(true); setNavbarModal(false); }}>🔑 Login</div>
            )}
          </div>
        )}
      </div>
      ...
      {login && (
        <Login key={Date.now()} setLoginModal={setLoginModal}
          onLoginSuccess={(name) => { setCurrentUser(name); setLogin(false); }} />
      )}
      ...
      {isListening && (
        <div style={{ position: "fixed", top: 0, left: 0, right: 0, bottom: 0, background: "rgba(0,0,0,0.7)", zIndex: 99999, display: "flex", alignItems: "center", justifyContent: "center", flexDirection: "column", gap: "20px" }}>
          <div style={{ background: "#212121", borderRadius: "16px", padding: "40px 60px", display: "flex", flexDirection: "column", alignItems: "center", gap: "20px", boxShadow: "0 8px 32px rgba(0,0,0,0.8)" }}>
            ...
            <button onClick={stopVoiceSearch} style={{ marginTop: "10px", padding: "8px 24px", borderRadius: "8px", border: "1px solid #555", background: "transparent", color: "white", cursor: "pointer", fontSize: "14px" }}>
              Cancel
            </button>
          </div>
        </div>
